...
export class Scripts {
  ...
  async addJob(
    ...
    const args = [queueKeys[''], job.name, job.timestamp];
    ...
    try {
      ...
      if (result < 0) {
        throw this.finishedErrors(result, job.id, 'addJob');
      }

      return result;
    } catch (err) {
      console.log('addJob exception', err);
    }
  }
  ...
  >(
    ...
    try {
      ...
      console.log('queue.addBulk exception:', err);
    }
  }
  ...
  moveToCompleted<T = any, R = any, N extends string = string>(
    ...
    const result = this.moveToFinished<T, R, N>(
      job,
      returnvalue,
      'returnvalue',
      removeOnComplete,
